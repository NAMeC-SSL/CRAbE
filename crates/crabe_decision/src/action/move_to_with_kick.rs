...
// #[derive(Clone)]
pub struct MoveToWithKick {
    pub move_to: MoveTo,
    pub should_kick: Box<dyn Fn(&World, &u8, &ToolData) -> Option<Kick>>

}

impl MoveToWithKick {
    pub fn new(target: Point2<f64>, orientation: f64, should_kick: Box<dyn Fn(&World, &u8, &ToolData) -> Option<Kick>>) -> Self
    {
        Self {
...
impl Action for MoveToWithKick {
    ...
    fn compute_order(&mut self, id: u8, world: &World, _tools: &mut ToolData) -> Command {
        ...
        cmd.kick = (self.should_kick)(world, &id, _tools);
